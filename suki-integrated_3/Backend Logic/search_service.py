...
def search_product_via_stores(search_key:str, stores_dict:dict, user_coordinates:tuple):
    ...
    # so the frontend can render the full list (used to seed the map on load).
    if not search_key:
        stores_with_product = {"exact_matches": list(stores_list), "related_matches": [], "has_results": True}
